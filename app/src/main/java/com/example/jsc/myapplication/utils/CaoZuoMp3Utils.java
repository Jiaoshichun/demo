...
public class CaoZuoMp3Utils {
     ...
     * @throws IOException
     */
    public static String heBingMp3(String hebingPath, String... paths) throws IOException {
        String luJing = "/storage/emulated/0/" + "HH音乐播放器/合并/";
        File f = new File(luJing);
        f.mkdirs();
        //生成处理后的文件
        File hebingFile = new File(luJing + hebingPath + "(HH合并).mp3");
        if (paths == null)
            return null;
        if (paths.length == 1) return paths[0];
        for (int i = 0; i < paths.length; i++) {
            File file = new File(fenLiData(paths[i]));
            FileInputStream in = new FileInputStream(file);
            FileOutputStream out = new FileOutputStream(hebingFile, i != 0);
            byte bs[] = new byte[1024 * 4];
            int len = 0;
            //先读第一个
        ...
        return hebingFile.getAbsolutePath();
    }
}
